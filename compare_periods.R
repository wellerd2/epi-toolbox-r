# compare_periods.R
# Compare model-based expected counts and incidence rates across two time periods.
#
# Operates on posterior model from the FoodNet Trends workflow (https://github.com/CDCgov/FoodNetTrends).
# The outputs should contain .draw, .epred, and population
# denominators. Within each draw, the function summarizes expected counts and
# incidence rates by period, computes rate ratios and percent change, and then
# aggregates results across draws using medians, quantiles, and HDIs.
#
# Required columns in `catch`:
# year, .draw, .epred, population, count
#
# Dependencies:
# dplyr, tidyr, bayestestR (hdi)

compare_periods <- function(catch,
                            baseline_start = 2007,
                            baseline_end   = 2014,
                            compare_start  = 2015,
                            compare_end    = 2024,
  ...
  # Basic checks
  req <- c("year", ".draw", ".epred", "population", "count")
  ...
  # Period-level summaries within each draw + period (median across years)
  period_draw <- dat %>%
    dplyr::group_by(.draw, period) %>%
    dplyr::summarise(
      period_epred = median(.epred, na.rm = TRUE),
      period_ir    = median(ir,     na.rm = TRUE),
      period_count = median(count,  na.rm = TRUE),
      period_pop   = median(population, na.rm = TRUE),
      .groups = "drop"
    )

  # Wide format: baseline vs compare within each draw
  wide <- period_draw %>%
    tidyr::pivot_wider(
      names_from = period,
  ...
  # Check both periods exist per draw
  wide <- wide %>%
    dplyr::filter(!is.na(period_ir_baseline) & !is.na(period_ir_compare))

  if (nrow(wide) == 0) stop("No draw rows have data in BOTH periods.")

  # Compute RR and percent change within each draw
  wide <- wide %>%
    dplyr::mutate(
  ...
  # Summarise across draws (single-row output)
  out <- wide %>%
    dplyr::summarise(
